...
//                                                    ЗАВДАННЯ 1
{
	...
	char text[10];                    //    Створюю масив символів
	cout << "Enter symbols : \n";


	for (int i = 1; i <= 10; i++)             //    Створюю цикл за допомогою якого буде записуватись масив символів
	{
		cin >> text[i];
		if (text[i] == '.')                     //        Якщо буде введено символ '.' , то введення даних буде припинено
		{
			break;
		}

	}
	for (int i = 1; i <= 10; i++)      // Для зручнішого інтерфейсу створюю новий цикл для роботи
	{
		if (text[i] == 'a')                    //  Якщо елемент масиву має значення 'a' , то їх кількість сумується
		{
			suma++;
		}

		if (text[i] == 'o')                      //Якщо елемент масиву має значення 'o' , то їх кількість сумується
		{
			suma2++;
	...
	max = suma2;                                         // Нехай максимальне значення = сумі символів 'o'

	if (suma > max)                                        // Якщо сума символів 'a' більша за максимальне, то вона і є максимальною
	{
		max = suma;
		cout << "Maksumym sumboliv is 'a' = " << suma << endl;
	}
	else if (suma2 > suma)                                                 // Якщо сума символів 'о' більша за суму символів 'a' , то вона і є максимальною
	{
		max = suma2;
